...
No R studio, vá até **Global option...** (Opçoes globais) 
...
Na opção  **Git/SVN** cheque se git está apontando para um arquivo **git.exe** em windows, em mac e linux **/usr/bin/git** ou **/usr/bin/gnome-terminal**  . Se você nunca fez isto, não deve ter nada no campo da chave RSA, clique Criar RSA Key (Create). O preenchimentos do campos que aparecem é opcional 
...
### Configurando o Github com Rstudio. 
Para copiar no GitHub, vá até o canto superior direito da sua página inicial do Perfil e selecione a opção **Settings**, em seguida vá na opção **SSH and GPG Keys** para colar e adicionar a chave. 
...
Nomei de forma informativa seu repositório, crie uma descrição igualmente informativa. Salecione a opção **Public** e opção **Add Readme** . Fialize clicando em **Create repository **
...
Indique o caminho onde seu projeto/repositório ficara salvo
...
Ao criar seu projeto desta forma, no seu Rstudio deve surgir uma aba chamada Git. E por onde vamos realizar o controle de versão (histórico de alterações dos nossos scritps)
...
Vamos criar uma estrutura de pastas para seu projeto. Isso facilitará o gereciamento do projeto. Análises de ENMs/SDMs envolvem muitos e diferentes tipos de arquivos (é fácil se perder se não houver essa organização).

```{r, echo=FALSE, out.width="35%", fig.align='center'}
knitr::include_graphics("figs/pastas_subpastas.png")
```

Uma estrutra simples envolve basicamente pastas para os dados, para os scripts, para os resultados, para documetos gerais (.doc, .pdf, .ppt) e para figuras. Dentro destas podem existem outras subpastas. A melhor organização fica a seu critério. 
---
...
#### Testando o controle de versão para nossos scripts R


Vamos criar um script de teste. Vá no no seguinte ícone clique R script
...
Digite **Teste** no seu arquivo R que até agora esta em branco. Em seguida salve este script (clicando noícone do disquete) com o nome "Teste" na pasta **R**, salvaremos todos os script R nesta pasta.  
...
Vá para aba Git e clique na opção R/
...
Adicione uma mensagem informativa e clique em Commite seguida em push.
...
```
Depois de dar Push, vá ate o Github e veja que seu script agora está lá. 
...
Adione qualquer texto ao seu script Teste e salve as alterações. 
...
### Organização dos projetos
Repita os passos anteriores clicando em Diff, adicionando uma mensagem informativa e em seguida clicar em Commit e finalizar clicando em Push
